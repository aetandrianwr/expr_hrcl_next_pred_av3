#!/usr/bin/env python3
"""
Enhanced Next-Location Prediction with Advanced Techniques
Target: >50% Acc@1 on GeoLife dataset
...
# ============================================================================
def analyze_dataset():
    """Auto-infer parameters from dataset."""
...
def get_loaders(batch_size=64):
    ...
    return (
        DataLoader(train, batch_size, shuffle=True, collate_fn=collate, pin_memory=True, num_workers=0),
        DataLoader(val, batch_size, shuffle=False, collate_fn=collate, pin_memory=True, num_workers=0),
        DataLoader(test, batch_size, shuffle=False, collate_fn=collate, pin_memory=True, num_workers=0)
    )
...
class DeepTransformer(nn.Module):
    """Deep transformer with hierarchical features and advanced pooling."""
    def __init__(self, vocab, d=128, h=8, L=4, drop=0.1):
        super().__init__()
        self.d = d
        
        # Embeddings - hierarchical concatenation
        d_loc = d // 2
        d_hier = d // 8
        self.loc = nn.Embedding(vocab['n_locs'], d_loc, padding_idx=0)
        self.s11 = nn.Embedding(vocab['s11'], d_hier, padding_idx=0)
        self.s13 = nn.Embedding(vocab['s13'], d_hier, padding_idx=0)
        self.s14 = nn.Embedding(vocab['s14'], d_hier, padding_idx=0)
        self.s15 = nn.Embedding(vocab['s15'], d_hier, padding_idx=0)
        ...
        # Project all features to d
        total_d = d_loc + 4*d_hier + 4*dt
    ...
    def forward(self, b):
        ...
        # Multi-scale pooling: last + attention weighted
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, smooth=0.1):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.smooth = smooth
    ...
    def forward(self, logits, tgt):
        ...
        # Focal loss
        ce = -smooth_tgt * log_p
        pt = torch.sum(smooth_tgt * p, dim=-1, keepdim=True)
        focal_weight = (1 - pt) ** self.gamma
        loss = (focal_weight * ce).sum(-1)
...
def train_model(model_class, vocab, config):
    ...
    crit = FocalLoss(smooth=config['smooth'])
...
def main():
    print("="*70)
    print("Enhanced Next-Location Prediction - Target: >50% Acc@1")
    print("="*70)
    
    vocab = analyze_dataset()
    dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {dev}\n")
    ...
    for i, cfg in enumerate(configs):
        ...
        test_metrics, model, n_params = train_model(model_class, vocab, cfg)
    ...
    if best_result['acc@1'] >= 50:
        ...
        print(f"\n⚠ Best result: {best_result['acc@1']:.2f}% - Continuing optimization...")
